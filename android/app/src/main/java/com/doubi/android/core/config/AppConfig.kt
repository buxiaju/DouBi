...
/**
 * DouBi Android 应用配置默认值（顶层 object）。
 *
 * 桌面版对照：`src/doubi/core/config.py:DEFAULTS`。
 *
 * **为什么放顶层不放 companion**：Kotlin 主构造器默认值的作用域解析规则下，
 * `data class AppConfig(... = DEFAULTS.xxx)` 看不到 `companion object` 里嵌的
 * `object DEFAULTS`（虽然能引用到同包 `object`，但语义不清且 IDE 也难追），
 * 放顶层一次性解决——既是单一真源，又被构造器 + ConfigValidator + tests
 * 三处直接看到。
 *
 * 外部访问（Kotlin 同包）：`DEFAULTS.xxx` 直接可见，无需 import。
 * 外部访问（跨包）：`import com.doubi.android.core.config.DEFAULTS` 然后用
 * `DEFAULTS.xxx`（不要写 `AppConfig.DEFAULTS.xxx`——已经不在 AppConfig 里了）。
 */
object DEFAULTS {
    // ---- 路径 / 输出 ----
 ...
 * DouBi Android 应用配置。
 *
 * 桌面版对照：`src/doubi/core/config.py:AppConfig`（30 字段）。
 ...
 * `core/config/AppConfigDataStore.updateField` 单独管理。
 *
 * 校验规则（与桌面版 `_validate_*` 系列函数对拍，详见 [ConfigValidator]）：
